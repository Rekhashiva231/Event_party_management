...
Public Class PaymentForm
    Dim pkVar As Long
    Private Sub SbTranTabForm_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Me.WindowState = FormWindowState.Maximized
        disRecords()

        If Conn.State = ConnectionState.Open Then Conn.Close()
        Conn.Open()
        Dim Cmd0 As New SqlCommand("select EventCode from EventTab order by EventCode", Conn)
    ...
    Private Sub butNew_Click(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        If Conn.State = ConnectionState.Open Then Conn.Close()
        Conn.Open()
        Dim Cmd0 As New SqlCommand("select max(TranNo) from PaymentTab", Conn)
    ...
    Sub SaveRecord()
        ...
        If Conn.State = ConnectionState.Open Then Conn.Close()
        Conn.Open()
        q1Var = "insert into PaymentTab("
        q2Var = " values("
        q1Var = q1Var & "TranNo" & ","
        q2Var = q2Var & "'" & UCase(TextBox1.Text) & "',"
        q1Var = q1Var & "TranDate" & ","
        q2Var = q2Var & "'" & Format(vDate.Value, "dd/MMM/yyyy") & "',"
        q1Var = q1Var & "EventCode" & ","
    ...
    Sub disRecords()
        ...
        Dim adp As New SqlDataAdapter("Select TranNo,EventCode,Amt From PaymentTab order by TranNo", Conn)
        adp.Fill(DS1)
    ...
    Private Sub DG1_CellContentClick(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles DG1.CellContentClick
        ...
        If Conn.State = ConnectionState.Open Then Conn.Close()
        Conn.Open()
        Dim Cmd0 As New SqlCommand("select * from PaymentTab where TranNo=" & pkVar & "", Conn)
        Dim D1 As SqlDataReader = Cmd0.ExecuteReader()
        If D1.HasRows Then
            D1.Read()
            TextBox1.Text = D1(0).ToString
            vDate.Value = D1(1)
            ComboBox1.Text = D1("EventCode").ToString
            TextBox3.Text = D1("Amt").ToString
    ...
    Private Sub butModify_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles butModify.Click
        ...
        If Conn.State = ConnectionState.Open Then Conn.Close()
        Conn.Open()
        Dim cmd1 As New SqlCommand("Delete from PaymentTab where TranNo=" & pkVar & "", Conn)
        cmd1.ExecuteNonQuery()
        If Conn.State = ConnectionState.Open Then Conn.Close()
        SaveRecord()
        butNew.Enabled = True
