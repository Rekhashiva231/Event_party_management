...
Public Class EnrollerDetailsForm
    Dim pkVar As String
    Private Sub EmpDetForm_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Me.WindowState = FormWindowState.Maximized
        If Conn.State = ConnectionState.Open Then Conn.Close()
        Conn.Open()
        Dim Cmd2 As New SqlCommand("select QualificationName from QualificationTab order by QualificationName", Conn)
    ...
    Sub SaveRecord()
        ...
        If Conn.State = ConnectionState.Open Then Conn.Close()
        Conn.Open()
        Dim Cmd0 As New SqlCommand("select EnrollerCode from EnrollerTab where EnrollerCode='" & UCase(TextBox1.Text) & "'", Conn)
        ...
        If Conn.State = ConnectionState.Open Then Conn.Close()
        Conn.Open()
        q1Var = "insert into EnrollerTab("
        q2Var = " values("
        q1Var = q1Var & "EnrollerCode" & ","
        q2Var = q2Var & "'" & UCase(TextBox1.Text) & "',"
        q1Var = q1Var & "pWord" & ","
        q2Var = q2Var & "'" & TextBox9.Text & "',"
        q1Var = q1Var & "EnrollerName" & ","
    ...
    Sub disRecords()
        ...
        Dim adp As New SqlDataAdapter("Select EnrollerCode,EnrollerName From EnrollerTab order by EnrollerCode", Conn)
        adp.Fill(DS1)
    ...
    Private Sub DG1_CellContentClick(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles DG1.CellContentClick
        ...
        If Conn.State = ConnectionState.Open Then Conn.Close()
        Conn.Open()
        Dim Cmd0 As New SqlCommand("select * from EnrollerTab where EnrollerCode='" & pkVar & "'", Conn)
        Dim D1 As SqlDataReader = Cmd0.ExecuteReader()
        If D1.HasRows Then
            D1.Read()
            TextBox1.Text = D1(0).ToString
            TextBox9.Text = D1(1).ToString
    ...
    Private Sub butModify_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles butModify.Click
        ...
        If Conn.State = ConnectionState.Open Then Conn.Close()
        Conn.Open()
        Dim cmd1 As New SqlCommand("Delete from EnrollerTab where EnrollerCode='" & pkVar & "'", Conn)
        cmd1.ExecuteNonQuery()
        If Conn.State = ConnectionState.Open Then Conn.Close()
        SaveRecord()
        butNew.Enabled = True
